...
function processSeen(seen) {
    ...
    for (var i = seen.pokemon.length - 1; i >= 0; i--) {
        ...
        if (!$('#seen_' + item.pokemon_id).length) addElement(item.pokemon_id, item.pokemon_name);
    ...
    for (var i = 1; i <= totalPokemon; i++) {
        if (shown.indexOf(i) < 0) $('#seen_' + i).hide();
    }
...
function addListeners(marker) {
    ...
    marker.addListener('mouseout', function () {
        if (!detailsPersist) $('#times_list').hide();
    });
...
if (location.href.match(/overlay_[0-9]+/g)) showOverlay(location.href.replace(/^.*overlay_([0-9]+).*$/, '$1'));
